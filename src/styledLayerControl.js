        ...
        if (this.options.collapsed) {
            ...
            if (!L.Browser.android) {
                L.DomEvent
                    ...
                    .on(container, 'mouseout', this._expand, this);
					 this._expand();
					 this._map.on('click', this._expand, this);
            }
    ...
    _onInputClick: function() {
        ...
        for (i = 0; i < inputsLen; i++) {
            ...
            if (input.checked && !this._map.hasLayer(obj.layer)) {
                this._map.addLayer(obj.layer);
				//console.log(obj.layer._map._leaflet_id);
				var nom_c=obj.name;
				//get_couche(nom_c,obj);
				if(nom_c!="Entité administrative")
				{					
				get_couche(obj);
				var couche_selectionner=liste_des_couches[obj.name];
				$("#stats_"+couche_selectionner).show();
				//console.log(obj.layer._map._leaflet_id);
				get_couche_allume("add",obj);	
				}				

            } else if (!input.checked && this._map.hasLayer(obj.layer)) {
                this._map.removeLayer(obj.layer);
				var couche_selectionner=liste_des_couches[obj.name];
				$("#stats_"+couche_selectionner).hide();
            }
        }
